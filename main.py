...
class Fire(Object):
    ...
    def __init__(self, x, y, width, height):
        ...
        self.fire = load_sprite_sheets("Traps", "Spikes", width, height)
...
def main():
    ...
    fire = Fire(100, HEIGHT - block_size - 31, 16, 32)
    fire.on()
    WIDTH = 2000
    floor = [Block(i * block_size, HEIGHT - block_size, block_size)                         #floor of blocks
             for i in range(-WIDTH // block_size, (WIDTH * 2) // block_size)]
    fire_floor = [Fire(i * 50, HEIGHT - block_size - 31, 16, 32)                         #floor of blocks
             for i in range(2, (WIDTH * 4) // block_size)]
    objects = [*floor, Block(0, HEIGHT - block_size * 2, block_size),                      #addss individual  block
               Block(block_size * 3, HEIGHT - block_size * 6, block_size), fire, *fire_floor]        #*
    ...
    blocks = [
              ...
              Block(block_size * -2, HEIGHT - block_size * 9, block_size),
                
              
              
              
              Block(block_size * 1.7, HEIGHT - block_size * 4.2, block_size),
              Block(block_size * 5, HEIGHT - block_size * 6, block_size),
              Block(block_size * 7, HEIGHT - block_size * 7, block_size),
              Block(block_size * 10, HEIGHT - block_size * 4.5, block_size),
              Block(block_size * 11, HEIGHT - block_size * 5, block_size),
              Block(block_size * 13, HEIGHT - block_size * 5, block_size),
              Block(block_size * 16, HEIGHT - block_size * 4.8, block_size),
              Block(block_size * 17.5, HEIGHT - block_size * 5, block_size),
              ...
              Block(block_size * 36, HEIGHT - block_size * 5, block_size),
              Block(block_size * 36, HEIGHT - block_size * 6, block_size),
              Block(block_size * 36, HEIGHT - block_size * 7, block_size),
              ]
    for i in blocks:
        objects.append(i)
    ...
    while run:
        ...
        if (((player.rect.right - offset_x >= WIDTH - scroll_area_width) and player.x_vel > 0) or (
                (player.rect.left - offset_x <= scroll_area_width) and player.x_vel < 0)) and (offset_x<2550):
